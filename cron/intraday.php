...
include_once($root_path . "/charts/includes/yql_keys.php");
...
// Check if we need to add new rows to the table (for a fresh new day) or if we are only refreshing intraday data
	$query = mysql_query("SELECT rundate, counter FROM myichi_tracker WHERE script='Intraday'");
	...
	//echo $updatecounter[rundate];
		if ($updatecounter[rundate] == $intraday) {
			echo "this is an Intraday refresh";
			$symbol_period_increment = 0;
			$queryType = "(Intraday refresh)";
			}
		else
			{
			...
			$queryType = "(Fresh new day!)";
			mysql_query("UPDATE myichi_tracker SET counter='$counter' WHERE script='Intraday'");
			}
...
// Get number of instruments available
	$query = mysql_query ("SELECT COUNT(*) AS num FROM myichi_instrument_names WHERE market IN ('AMS',  'BRU',  'LIS', 'PAR')");
...
// Get the list of Yahoo_symbols and convert them into a YQL useable string
		$query = mysql_query ("SELECT yahoo_symbol FROM myichi_instrument_names WHERE market IN ('AMS',  'BRU',  'LIS', 'PAR') LIMIT $selectid,$selectidGroupSize ") or die ('Erreur de requête<br />'.$sql.'<br />'.mysql_error());
...
$info = 'Query type: ' . $queryType . ' EU stock';
$end = date('H:i:s', time());
mysql_query("UPDATE myichi_tracker SET rundate='$intraday', counter='$counter' WHERE script='Intraday' ");
mysql_query("INSERT INTO myichi_tracker_report (script, date, start, end, yqlqty, records, info) VALUES ('Intraday', '$intraday', '$start', '$end', '$yqlqty', '$addedRecordsCounter', '$info')");
...
// Sent email with results:
if ($cronMailIntradayResults > 0) {
     $subject = 'Intraday data update results';
     $message = '
	 <html><body>Just added ' . $addedRecordsCounter . ' records with Intraday ' . $queryType . ' data from ' . $intraday .' to the database. <br>'
